    ...
    if request.user.is_authenticated:
        ...
        # Permit all users in this strand
            ...
            if request.user.role.name == 'admin' or request.user.participant_strand.name == self.model._meta.app_label:
                return True
        # Permit all users in this strand if object was created within the past week
        elif permission == 'all_users_in_strand_recently_created':
            if (request.user.role.name == 'admin' or request.user.participant_strand.name == self.model._meta.app_label) and (obj and date.today() < (obj.created.date() + timedelta(days=7))):
                return True
